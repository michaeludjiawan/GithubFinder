...
import com.michaeludjiawan.githubfinder.util.toVisibility
...
class UserLoadStateViewHolder(
    ...
    init {
        view.tv_load_state_retry_button.setOnClickListener { retry.invoke() }
    }

    fun bind(loadState: LoadState) {
        if (loadState is LoadState.Error) {
            view.tv_load_state_error_msg.text = loadState.error.localizedMessage
        }
        view.tv_load_state_progress_bar.visibility = toVisibility(loadState is LoadState.Loading)
        view.tv_load_state_retry_button.visibility = toVisibility(loadState !is LoadState.Loading)
        view.tv_load_state_error_msg.visibility = toVisibility(loadState !is LoadState.Loading)
    }
